...
def ensure_document_is_valid(document: str) -> None:
    ...
    if document != document.strip():
        raise podgenai.exceptions.InputError("Document contains leading or trailing whitespace.")

    if len(document) == 0:
        raise podgenai.exceptions.InputError("Document is empty.")
    ...
    for model in models:
        ...
        if document_token_count > text_model_input_token_capacity:
            ...
            raise podgenai.exceptions.InputError(f"Document uses {document_token_count:,} tokens, but the {model['name']} model's input capacity is {text_model_input_token_capacity:,} tokens. Reduce the document size by at least {excess_tokens:,} tokens ({reduction_ratio:.0%}).")
