...
from podgenai.types import SpeakerCount
...
@click.option("--document", "-d", type=Path, default=None, help="Path to a single text or markdown document file to use as the exclusive source for the podcast. If not given, the podcast is generated from the model's internal knowledge.")
...
def main(topic: str | None, path: Path | None, document: Path | None, max_sections: int | None, speakers: SpeakerCount, markers: bool, confirm: bool) -> None:
    ...
    try:
        ...
        if document:
            assert isinstance(document, Path), (document, type(document))
            if not document.is_file():
                raise podgenai.exceptions.InputError(f"Document path is not a file: {document}")
            try:
                document_text = document.read_text()
            except UnicodeDecodeError:  # Observed for PDF and DOCX files.
                raise podgenai.exceptions.InputError(f"Document path is not readable as a text file: {document}")
        ...
        generate_media(topic, output_path=path, document=document_text, max_sections=max_sections, speakers=speakers, markers=markers, confirm=confirm)
