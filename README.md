# podgenai
**podgenai** is a Python 3.12 application to generate an informational single-speaker audiobook/podcast mp3 file on a given topic using the GPT-4 LLM. The typical duration of the generated file is 30 to 90 minutes. A funded [OpenAI API key](https://platform.openai.com/api-keys) is required.
...
## Usage
Usage can be as a command-line application or as a Python library. By default, the generated mp3 file will be written to the current working directory. As of 2024, the typical cost per generation is under 3 USD, more specifically under 0.10 USD per subtopic. The time taken is under three minutes.
